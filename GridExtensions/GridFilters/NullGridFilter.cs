namespace GridExtensions.GridFilters
{
    ...
    {
        internal const string FilterFormat = "Convert(ISNULL({0}, '" + DummyStringValue + "'), System.String) {1} '"
                                             + DummyStringValue + "'";

        internal const string FilterRegex = @"Convert\(ISNULL\(\[[a-zA-Z].*\], '" + DummyStringValue
                                            + @"'\), System.String\) (?<Operator>(=|<>)) '" + DummyStringValue + "'";

        private const string DummyStringValue = "a§df43dj§öap";
        ...
        public override void SetFilter(string filter)
        {
            ...
            if (regex.IsMatch(filter))
            {
                ...
                this.checkBox.CheckState = CheckState.Indeterminate;
                this.checkBox.CheckState = match.Groups["Operator"].Value == "="
